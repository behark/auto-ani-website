...
// Simple GET endpoint that triggers seeding for testing
    ...
    if (currentCount === 0) {
      ...
      for (const vehicle of vehicles) {
        try {
          ...
          results.push(`✅ Added: ${vehicle.make} ${vehicle.model} (${vehicle.year})`);
        } catch (error) {
          console.error(`❌ Failed to add ${vehicle.make} ${vehicle.model}:`, error);
          results.push(`❌ Failed: ${vehicle.make} ${vehicle.model} - ${error instanceof Error ? error.message : String(error)}`);
        }
      }
    }
