    ...
    import('web-vitals').then(({ onCLS, onINP, onFCP, onLCP, onTTFB }) => {
      const reportMetric = (metric: any) => {
        ...
        if (typeof window !== 'undefined' && debug) {
          ...
          localStorage.setItem('webVitals', JSON.stringify(metrics.slice(-10))); // Keep last 10
        }
      };
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      const logMetrics = () => {
        if (metrics.length > 0) {
          ...
          metrics.forEach(metric => {
            ...
            console.log(`${status} ${metric.name}: ${metric.value.toFixed(2)}${metric.name === 'CLS' ? '' : 'ms'}`);
          });
          console.groupEnd();
        }
      };
      ...
      import('web-vitals').then(({ onCLS, onINP, onFCP, onLCP, onTTFB }) => {
        const collectMetric = (metric: any) => {
          metrics.push(metric);
          if (metrics.length === 5) { // When all core metrics are collected
