...
export default function EnhancedAboutPage() {
    ...
    async function fetchData() {
      try {
        ...
        if (teamData && teamData.length > 0) {
          ...
          setTeamMembers(fallbackTeamMembers.map((member, index) => ({
            _id: `fallback-${index}`,
            name: member.name,
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-20">
        ...
        <div className="relative z-10 container mx-auto px-4 h-full flex items-center">
          <div className="text-white max-w-2xl">
            <h1 className="text-5xl font-bold mb-4">
              {t('about.title')} <span className="text-[var(--primary-orange)]">{displayBusinessName}</span>
            </h1>
            <p className="text-xl">
              {displayDescription}
            </p>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <div className="mt-8 grid grid-cols-2 gap-4">
                ...
                <Card>
                  <CardContent className="p-4 text-center">
                    <div className="text-3xl font-bold text-[var(--primary-orange)]">{yearsInBusiness}+</div>
      ...
      {businessInfo && businessInfo.certifications && businessInfo.certifications.length > 0 && (
        <section className="py-16">
          <div className="container mx-auto px-4">
            <div className="text-center mb-12">
              <h2 className="text-3xl font-bold mb-4">Certifications & Languages</h2>
            </div>
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <Card>
                <CardContent className="p-6">
                  <h3 className="text-xl font-semibold mb-4">Languages We Speak</h3>
            ...
            {teamMembers.map((member, index) => (
              <Card key={member._id} className="overflow-hidden">
                <div className="aspect-square bg-gray-200 relative">
                  <FallbackImage
                    src={member.image && member.image.asset ? urlFor(member.image).width(300).height(300).url() : '/images/placeholder-vehicle.svg'}
                    alt={member.name}
                    fill
                    className="object-cover"
                ...
                <CardContent className="p-4 text-center">
                  ...
                  <p className="text-sm text-[var(--primary-orange)]">{member.role}</p>
                  {member.experience && (
                  ...
                  {member.languages && member.languages.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2 justify-center">
                      {member.languages.slice(0, 2).map((lang, langIndex) => (
                        <span key={langIndex} className="text-xs bg-gray-100 px-2 py-1 rounded">
                          {lang}
                        </span>
                      ))}
                    </div>
